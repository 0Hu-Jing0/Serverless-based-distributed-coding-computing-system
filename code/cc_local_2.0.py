...
# run
async def matrix_multipl(column, group, code, matrix1, matrix2):
    return column, group, code, np.dot(matrix1,matrix2)
...
for j in range(column_B):
    ...
    for i in range(row_A // 2):
        ...
        tasks.append(matrix_multipl(j, i, 0, A[2 * i], B_trans[j].T))
        tasks.append(matrix_multipl(j, i, 1, A[2 * i + 1], B_trans[j].T))
        tasks.append(matrix_multipl(j, i, 2, A[2 * i] + A[2 * i + 1], B_trans[j].T))
...
for fut in done:
    column, group, code, num = fut.result()
